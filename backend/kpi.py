...
def get_unresolved_percentage(db: Session = Depends(get_db)):
    ...
    result_total = get_all_tickets(db)
    total_tickets = result_total["value"]
...
def get_resolved_tickets(db:Session = Depends(get_db)):
    resolved_query = text (
    """
...
def get_resolved_percentage(db: Session = Depends(get_db)):
    resolved_count = get_resolved_tickets(db)
    result_total = get_all_tickets(db)
    total_tickets = result_total["value"]
...
def get_overdue_percentage(db: Session = Depends(get_db)):
    ...
    total_tickets = get_resolved_tickets(db)
